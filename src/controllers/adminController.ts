...
export class AdminController {
    ...
    static async createUser(request: IJwtRequest, response: Response): Promise<void> {
        ...
        try {
            ...
            sendSuccess(response, { user: result.data!.user }, 'User created successfully by admin');
        } catch (error) {
            console.error('Admin create user error:', error);
            sendError(response, 500, 'Failed to create user', ErrorCodes.SRVR_DATABASE_ERROR);
        }
    }
    ...
    static async getAllUsers(request: IJwtRequest, response: Response): Promise<void> {
        ...
        try {
            const result = await adminService.getAllUsers({ page, limit, status, role: (role !== undefined && !isNaN(role) && role >= 1 && role <= 5) ? role : undefined });

            const filterDesc = result.data.filters ? ' with filters applied' : '';
            sendSuccess(response, result.data, `Retrieved ${result.data.pagination.totalUsers} users${filterDesc}`);
        } catch (error) {
    ...
    static async searchUsers(request: IJwtRequest, response: Response): Promise<void> {
        ...
        const searchFields = fieldsParam ? fieldsParam.split(',').map(f => f.trim()) : ['firstname', 'lastname', 'username', 'email'];
        ...
        try {
            const result = await adminService.searchUsers({ searchTerm, fields: fieldsToSearch, page, limit });
            sendSuccess(response, result.data, `Found ${result.data!.pagination.totalUsers} users matching "${searchTerm}"`);
        } catch (error) {
    ...
    static async getUserById(request: IJwtRequest, response: Response): Promise<void> {
        ...
        try {
            ...
            sendSuccess(response, result.data, 'User details retrieved successfully');
        } catch (error) {
    ...
    static async updateUser(request: IJwtRequest, response: Response): Promise<void> {
        ...
        if (accountStatus === undefined && emailVerified === undefined && phoneVerified === undefined) {
            sendError(response, 400, 'No valid updates provided', ErrorCodes.VALD_MISSING_FIELDS);
            return;
        }

        try {
            const result = await adminService.updateUser(userId, { accountStatus, emailVerified, phoneVerified });
    ...
    static async getDashboardStats(request: IJwtRequest, response: Response): Promise<void> {
        try {
            ...
            sendSuccess(response, result.data, 'Dashboard statistics retrieved');
        } catch (error) {
    ...
    static async changeUserRole(request: IJwtRequest, response: Response): Promise<void> {
        ...
        try {
            ...
            sendSuccess(response, result.data, `User role changed from ${prevRole.role} to ${RoleName[newRole as UserRole]}`);
        } catch (error) {
